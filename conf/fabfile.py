def production():
    ...
    set(fab_user="taylan")
    set(remote_dir="/home/taylan/sites/orangeblog")
...
def deploy(hash="HEAD"):
    ...
    local("mkdir ../tmp")
    local("cd ..; git archive --format=tar --prefix=deploy/ $(hash) conf build/libs build/orangeblog | gzip > tmp/archive.tar.gz")
    
    # Untar the archive to minify js files
    local("cd ../tmp; tar -xzf archive.tar.gz; rm -f archive.tar.gz")
    local("python /usr/local/lib/yuicompressor/bin/jsminify.py --dir=../tmp/deploy/build/orangeblog/media/assets/js")
    ...
    run("cd $(remote_dir)/deploy/orangeblog; ln -s ../conf/settings.py settings_local.py")
